...
struct CompareParameters {
    ...
    std::string              regionsFile;     // optional BED file of regions
                                              // to restrict comparison to
    std::string              regionsString;   // optional CLI region expression
                                              // e.g. "chr1:1000000-2000000"
                                              // or "chr1,chr2:5000-10000"
...
private:
    ...
    // Per-chromosome sorted half-open intervals [start, end).
    // Empty => no region filter (whole genome).
    std::map<std::string, std::vector<std::pair<int32_t,int32_t>>> regions;
    bool                                                            hasRegions = false;

    // Parse --regions / --regions-bed into the `regions` map.
    // Returns false on parse error.
    bool loadRegions();

    // Returns true iff the given 0-based position falls inside any
    // region on that chromosome.  When hasRegions == false always true.
    bool inRegion(const std::string& chrom, int32_t pos) const;
