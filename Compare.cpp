...
int CompareMain(int argc, char** argv, std::string version)
{
    ...
    static struct option long_opts[] = {
        ...
        {"regions",            required_argument, 0,  4 },
        {"regions-bed",        required_argument, 0,  5 },
        {0, 0, 0, 0}
    };
    ...
    while ((opt = getopt_long(argc, argv, "ho:t:n:s:", long_opts, &idx)) != -1) {
        switch (opt) {
            ...
            case  4 : params.regionsString = optarg;             break;
            case  5 : params.regionsFile   = optarg;             break;
    ...
    std::cerr << "Regions (CLI)      : " << (params.regionsString.empty() ? "<all>" : params.regionsString) << "\n";
    std::cerr << "Regions BED        : " << (params.regionsFile.empty() ? "<none>" : params.regionsFile) << "\n";
    std::cerr << "VCF files          :\n";
    for (size_t i = 0; i < params.vcfFiles.size(); ++i)
