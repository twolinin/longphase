...
// ====================================================================
//  Region filter – support BED file or inline CLI expression
// ====================================================================
//
//  CLI syntax for --regions:
//      chrN                      whole chromosome
//      chrN:start-end            1-based inclusive (htslib/samtools style)
//      chrN:start                single position -> [start,start]
//      Multiple regions: comma-separated.
//
//  BED file syntax (via --regions-bed):
//      chrom\tstart\tend         0-based half-open (standard BED)
//
//  Internally we always store 0-based half-open [start, end) intervals.
//  Empty string for both -> no filter, whole genome.

namespace {
bool parseRegionExpr(const std::string& expr,
                     std::map<std::string, std::vector<std::pair<int32_t,int32_t>>>& out)
{
    // Split on commas
    size_t p = 0;
    while (p < expr.size()) {
        size_t q = expr.find(',', p);
        if (q == std::string::npos) q = expr.size();
        std::string tok = expr.substr(p, q - p);
        p = q + 1;
        ...
        if (colon == std::string::npos) {
            // whole chromosome -> huge interval
            out[tok].push_back({0, INT32_MAX});
            continue;
        }
        std::string chr = tok.substr(0, colon);
        std::string rng = tok.substr(colon + 1);

        // "start-end" or "start"
        size_t dash = rng.find('-');
        long long start1 = 0, end1 = 0;
        try {
            if (dash == std::string::npos) {
                start1 = std::stoll(rng);
                end1   = start1;
            } else {
                start1 = std::stoll(rng.substr(0, dash));
                end1   = std::stoll(rng.substr(dash + 1));
            }
        } catch (...) {
            std::cerr << "[compare] ERROR: cannot parse region '"
                      << tok << "'\n";
            return false;
        }
        if (start1 < 1 || end1 < start1) {
            std::cerr << "[compare] ERROR: invalid region '"
                      << tok << "'\n";
            return false;
        }
        // 1-based inclusive -> 0-based half-open
        out[chr].push_back({(int32_t)(start1 - 1), (int32_t)end1});
    }
    return true;
}
}  // anonymous namespace

bool CompareProcess::loadRegions()
{
    // No filter requested
    if (params.regionsFile.empty() && params.regionsString.empty()) {
        hasRegions = false;
        return true;
    }

    // BED file
    if (!params.regionsFile.empty()) {
        std::ifstream ifs(params.regionsFile);
        if (!ifs) {
            std::cerr << "[compare] ERROR: cannot open regions BED: "
                      << params.regionsFile << "\n";
            return false;
        }
        std::string line;
        ...
        while (std::getline(ifs, line)) {
            ...
            if (line.empty() || line[0] == '#' ||
                line.rfind("track", 0) == 0 ||
                line.rfind("browser", 0) == 0) continue;
            std::istringstream iss(line);
            std::string chrom; long long s = 0, e = 0;
            if (!(iss >> chrom >> s >> e)) {
                std::cerr << "[compare] ERROR: malformed BED line "
                          << lineNum << ": " << line << "\n";
                return false;
            }
            if (s < 0 || e < s) {
                std::cerr << "[compare] ERROR: invalid BED interval "
                             "on line " << lineNum << "\n";
                return false;
            }
            regions[chrom].push_back({(int32_t)s, (int32_t)e});
        }
    }

    // CLI region expression (may be combined with --regions-bed)
    if (!params.regionsString.empty()) {
        if (!parseRegionExpr(params.regionsString, regions)) return false;
    }

    // Merge overlapping intervals per chromosome for fast lookup
    long long totalBp = 0;
    size_t    totalN  = 0;
    for (auto& kv : regions) {
        auto& v = kv.second;
        ...
        v = std::move(merged);
        for (auto& iv : v) totalBp += (long long)(iv.second - iv.first);
        totalN += v.size();
    }
    hasRegions = true;
    std::cerr << "[compare] region filter: " << totalN
              << " intervals across " << regions.size()
              << " chromosomes, total "
              << totalBp << " bp\n";
    return true;
}

bool CompareProcess::inRegion(const std::string& chrom, int32_t pos) const
{
    if (!hasRegions) return true;
    auto it = regions.find(chrom);
    if (it == regions.end()) return false;
    const auto& v = it->second;
    // Binary search for the first interval whose end > pos
    auto hit = std::upper_bound(
        v.begin(), v.end(), std::make_pair(pos, INT32_MAX),
        [](const std::pair<int32_t,int32_t>& a,
           const std::pair<int32_t,int32_t>& b) {
            return a.first < b.first;
        });
    // The candidate interval is hit-1 (the last interval with start <= pos)
    if (hit == v.begin()) return false;
    --hit;
    return pos >= hit->first && pos < hit->second;
}
                             ...
                             long long& outTotalPhasedSnv)
{
    ...
    while (bcf_read(fp, hdr, rec) >= 0) {
        ...
        // Region filter – skip variant if outside any requested interval
        {
            const char* chr = bcf_hdr_id2name(hdr, rec->rid);
            if (!inRegion(chr, v.pos)) continue;
        }
        ...
        const std::vector<VariantRecord>& query)
{
    ...
    //
    //  Blocks are computed STRICTLY per chromosome.  `psPos` is a local
    //  variable created fresh for each call to compareChromosome(), so
    //  even if the same PS id appears on multiple chromosomes (which
    //  would be unusual) it cannot be merged across chromosomes here.
    //
    {
...
int CompareProcess::run()
{
    ...
    //
    //  IMPORTANT: blocks are formed INSIDE each chromosome.  We never
    //  merge variants with the same PS across chromosomes – PS is only
    //  unique within one chromosome by VCF convention, and concatenated
    //  VCFs from different samples can break that.
    //
    //  We additionally track which chromosomes each PS id was observed
    //  on, so we can warn the user if PS values are reused across
    //  chromosomes (typically a sign of a malformed or merged VCF).
    long long queryTotalBlocks = 0, queryTotalBlockSum = 0, queryTotalBlockN50 = 0;
    {
        std::vector<long long> allSpans;
        std::unordered_map<int32_t, std::set<std::string>> psChroms;
        ...
        // Check for cross-chromosome PS reuse (defensive: VCF spec says
        // PS is unique within a sample but does NOT promise uniqueness
        // across chromosomes; some tools emit increasing global counters,
        // others restart per chromosome).  We always treat blocks as
        // chromosome-local here, but inform the user if the assumption
        // matters.
        size_t collisions = 0;
        std::vector<int32_t> sample;
        for (const auto& kv : psChroms) {
            if (kv.second.size() >= 2) {
                ++collisions;
                if (sample.size() < 3) sample.push_back(kv.first);
            }
        }
        if (collisions > 0) {
            std::cerr << "[compare] Notice: " << collisions
                      << " PS id(s) in " << queryPath
                      << " are reused across multiple chromosomes ";
            std::cerr << "(e.g. PS=";
            for (size_t i = 0; i < sample.size(); ++i)
                std::cerr << (i ? "," : "") << sample[i];
            std::cerr << ").  Blocks are still computed per-chromosome, "
